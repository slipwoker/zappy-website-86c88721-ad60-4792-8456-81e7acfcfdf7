      ...
      if (!window.zappyContactFormLoaded) {
        try {
          ...
          const response = await fetch('http://localhost:5001/api/email/contact-form', {
            ...
            body: JSON.stringify({
              websiteId: '86c88721-ad60-4792-8456-81e7acfcfdf7',
              name: formData.name,
              email: formData.email,
              subject: formData.service ? `שירות: ${formData.service}` : 'פנייה מטופס יצירת קשר',
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "שלווה רוחנית משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
